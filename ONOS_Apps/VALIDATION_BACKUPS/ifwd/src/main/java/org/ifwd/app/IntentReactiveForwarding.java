...
public class IntentReactiveForwarding {
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
	    ...
	    log.info(ethPkt.toString());
            ...
            setUpConnectivity(context, srcId, dstId, ethPkt);
	    setUpConnectivity(context, dstId, srcId, ethPkt);
    ...
    private void setUpConnectivity(PacketContext context, HostId srcId, HostId dstId, Ethernet ethPkt) {
        
	TrafficSelector.Builder selector_b;
	...
	if (ethPkt.getEtherType() == Ethernet.TYPE_ARP) {
		selector_b = DefaultTrafficSelector.builder()
                                        .matchEthSrc(ethPkt.getSourceMAC())
                                        .matchEthDst(ethPkt.getDestinationMAC())
                                        .matchEthType(ethPkt.getEtherType());
                                        //.matchIPProtocol(((IPv4) ethPkt.getPayload()).getProtocol());
                selector = selector_b.build();
        ...
        /*if (srcId.toString().compareTo(dstId.toString()) < 0) {
            ...
            key = Key.of(dstId.toString() + srcId.toString(), appId);
                ...
                key = Key.of(srcId.toString() + dstId.toString() + "ARP", appId);
	} else {
		if (TpPort.tpPort(((TCP) ((IPv4) ethPkt.getPayload()).getPayload()).getDestinationPort()).toInt() == 80) {
		...
		selector_b = DefaultTrafficSelector.builder()
					...
					.matchEthType(ethPkt.getEtherType())
					.matchIPProtocol(((IPv4) ethPkt.getPayload()).getProtocol())
					.matchIPDst(IpPrefix.valueOf(((IPv4) ethPkt.getPayload()).getDestinationAddress(), 0))
					.matchIPSrc(IpPrefix.valueOf(((IPv4) ethPkt.getPayload()).getSourceAddress(), 0))
					.matchTcpDst(TpPort.tpPort(((TCP) ((IPv4) ethPkt.getPayload()).getPayload()).getDestinationPort()));
		selector = selector_b.build();
		treatment = DefaultTrafficTreatment.emptyTreatment();
		key = Key.of(srcId.toString() + dstId.toString() + TpPort.tpPort(((TCP) ((IPv4) ethPkt.getPayload()).getPayload()).getDestinationPort()).toString(), appId);
		...
		selector_b = DefaultTrafficSelector.builder()
                                        ...
                                        .matchIPDst(IpPrefix.valueOf(((IPv4) ethPkt.getPayload()).getDestinationAddress(), 0))
                                        .matchIPSrc(IpPrefix.valueOf(((IPv4) ethPkt.getPayload()).getSourceAddress(), 0))
                                        .matchTcpSrc(TpPort.tpPort(((TCP) ((IPv4) ethPkt.getPayload()).getPayload()).getSourcePort()));
                selector = selector_b.build();
                treatment = DefaultTrafficTreatment.emptyTreatment();
		key = Key.of(srcId.toString() + dstId.toString(), appId);
		...
		//key = Key.of(srcId.toString() + dstId.toString() + ((IPv4) ethPkt.getPayload()).getSourceAddress() + TpPort.tpPort(((TCP) ((IPv4) ethPkt.getPayload()).getPayload()).getDestinationPort()).toString(), appId);
	}
